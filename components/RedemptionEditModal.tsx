...
const EMPTY_CALC: RedemptionCalculations = {
  ...
  isShortTerm: false,
  returnPerUnit: 0,
  proratedPreferredReturn: 0,
  aggregatePurchasePrice: 0,
  isEarlyExit: false,
  completedMonthsDistributed: 0,
  distributionClawback: 0,
  netAggregatePrice: 0,
};
...
export function RedemptionEditModal({
  ...
  useEffect(() => {
    adminApi.redemption(redemptionId).then(async (r) => {
      if (!r.success || !r.data) {
        setMsg("Failed to load redemption.");
        setLoading(false);
        return;
      }
      const d = r.data;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      adminApi
        .getRedemptionPreview(trancheId, units, form.effectiveDate!)
    ...
    >
      ...
      >
        ...
        <div style={{ padding: "20px 24px" }}>
          {loading || !form ? (
            <div style={{ padding: 20, color: msg ? "#b91c1c" : "#64748b" }}>
              {msg || "Loading..."}
            </div>
          ) : (
            <form onSubmit={submit}>
              {form.trancheApplicationId && (
                ...
                >
                  Linked to Investment #{form.trancheApplicationId} (link not
                  editable from this page — use the investor&apos;s user page to
                  change)
                </div>
              )}
              ...
              >
                ...
                <div>
                  ...
                  <div style={readOnlyBoxStyle}>
                    {form.totalUnitsOwned || "—"}
                  </div>
                </div>
                <div>
                  <label style={labelStyle}>Status</label>
                  <div style={readOnlyBoxStyle}>
                    {(form.status === "Active" ? "Redeemed" : form.status) ||
                      "Redeemed"}
                  </div>
                </div>
              </div>
              ...
              >
                <FormField label="Units to Redeem *">
                  <input
                    ...
                    onChange={(e) =>
                      setForm(
                        (f) => f && { ...f, unitsToRedeem: e.target.value },
                      )
                    }
                  />
                </FormField>
                <FormField label="Effective Date *">
                  <input
                    ...
                    onChange={(e) =>
                      setForm(
                        (f) => f && { ...f, effectiveDate: e.target.value },
                      )
                    }
                  />
                </FormField>
