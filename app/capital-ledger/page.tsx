...
const sortValue = (
  ...
  switch (field) {
    ...
    case "investorName":
      return e.investorName.toLowerCase();
    case "accountUserName":
      return e.accountUserName.toLowerCase();
    case "units":
      return e.units ?? -Infinity;
    case "amount":
      return e.amount;
    case "dividendPaid":
      return e.dividendPaid ?? -Infinity;
    case "runningBalance":
...
function CapitalLedgerContent() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    adminApi
      .capitalLedger({ from: from || undefined, to: to || undefined })
      .then((r) => {
  ...
  const visibleEntries = (data?.entries ?? []).filter((e) => {
    ...
    if (search) {
      ...
      if (
        ...
        !e.accountUserEmail.toLowerCase().includes(q)
      )
        return false;
    }
    if (appIdFilter) {
  ...
  useEffect(() => {
    setPage(1);
  }, [data, typeFilter, search, appIdFilter, sortField, sortDir]);
  ...
  const exportToExcel = async () => {
    setExporting(true);
    const r = await adminApi.capitalLedger({
      from: from || undefined,
      to: to || undefined,
    });
    if (r.success && r.data) {
      const rows = r.data.entries
        .filter(
          (e) => activeTypes.length === 0 || activeTypes.includes(e.entryType),
        )
        .map((e) => [
          entryLabel(e),
      ...
      <div style={{ padding: "32px 36px" }}>
        ...
        >
          <div>
            ...
            <p style={{ color: "#64748b", fontSize: 13 }}>
              Chronological record of all fund capital movements —
              contributions, redemptions, and dividends — with running balance.
            </p>
          </div>
        </div>
        ...
        {data && (
          ...
          >
            {data.openingBalance !== 0 &&
              statCard(
                "Opening Balance",
                fmtFull(data.openingBalance),
                "#0f2342",
                from ? `as at ${from}` : undefined,
              )}
            {statCard(
              "Total Contributions",
              fmtFull(data.totalContributions),
              "#15803d",
              `${data.entries.filter((e) => e.amount > 0).length} entries`,
            )}
            {statCard(
              "Total Redemptions",
              `−${fmtFull(data.totalRedemptions)}`,
              "#be123c",
              `${data.entries.filter((e) => e.entryType === "Redemption").length} entries`,
            )}
            {statCard(
              "Dividends Paid",
              `−${fmtFull(data.totalDistributions + data.totalRedemptionDistributions)}`,
              "#b45309",
            ...
            {statCard(
              "Net Balance",
              fmtFull(data.closingBalance),
              "#0f2342",
              "closing balance",
            )}
            {data.totalPendingAccruals > 0 &&
              statCard(
                "Accrued & Unpaid",
                `~${fmtFull(data.totalPendingAccruals)}`,
                "#7c3aed",
                "pending daily accruals",
              )}
          </div>
        )}
        ...
        {runningBalanceUnreliable && (
          ...
          >
            ...
            >
              <strong>Running Balance</strong> is each row&rsquo;s true fund
              balance as of that date, but it only <em>builds up</em> correctly
              oldest → newest with every row shown. A search, App ID, or type
              filter hides rows, and any sort other than Date (oldest first)
              reorders them — either way, consecutive balances won&rsquo;t add
              up. Clear filters and sort by Date ascending to see it build up in
              order. (The From/To date range is fine on its own — the Opening
              Balance card above already accounts for it.)
            </span>
            <button
              onClick={() => {
                setTypeFilter("");
