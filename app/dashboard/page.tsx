...
function BalanceFlow({ stats }: { stats: DashboardStats }) {
  ...
  const remaining =
    stats.totalDeployedCommencement - stats.totalWithdrawnCommencement;
  ...
  const variance =
    hasDeployed && hasBank
      ? (stats.bankAccountBalance ?? 0) - (available ?? 0)
      : null;
  ...
  const hover = (href?: string) => ({
    onMouseEnter: (e: React.MouseEvent<HTMLDivElement>) => {
      if (href) e.currentTarget.style.boxShadow = "0 4px 16px rgba(0,0,0,0.10)";
    },
    onMouseLeave: (e: React.MouseEvent<HTMLDivElement>) => {
      if (href) e.currentTarget.style.boxShadow = "";
    },
  });
    ...
    >
      ...
      >
        ...
        {box(
          ...
          fmt(stats.totalDeployedCommencement),
          "#0e3416",
          false,
          "/capital-ledger?type=Contribution",
        )}
        {arrow(
          "Redeemed",
          `−${fmt(stats.totalWithdrawnCommencement)}`,
          "#ef4444",
          false,
          "/capital-ledger?type=Redemption",
        )}
        {box("Balance Remaining", fmt(remaining), "#6366f1")}
        {arrow(
          "Dividend Paid",
          `−${fmt(stats.interestPaidCommencement)}`,
          "#f59e0b",
          false,
          "/capital-ledger?type=Redemption,Dividend",
        ...
        {box(
          "Sponsored Equity",
          hasSponsoredEquity ? fmt(stats.sponsoredEquity ?? 0) : "Not entered",
          "#699172",
          !hasSponsoredEquity,
          "/settings",
        )}
        ...
        {/* Row 3 */}
        {box(
          "Profit Received from Bank",
          hasInterestReceived
            ? fmt(stats.interestReceived ?? 0)
            : "Not entered",
          "#0f2342",
          !hasInterestReceived,
          "/settings",
        ...
        {arrow(
          ...
          hasOtherCharges ? `−${fmt(stats.otherCharges ?? 0)}` : "Not entered",
          "#ef4444",
          !hasOtherCharges,
          "/settings",
        )}
      ...
      {!hasDeployed && (
        <div style={{ fontSize: 11, color: "#b8923a", marginTop: 10 }}>
          ⚠ Enter today&apos;s Deployed Amount in Admin → Settings → Daily
          Balances to complete this flow.
        </div>
      )}
    </div>
  );
